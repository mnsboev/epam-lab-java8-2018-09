...
class Exercise2 {
    ...
    void employersStuffList() {
        ...
        Map<String, Set<Person>> result = employees.stream()
                ...
                .collect(groupingBy(emp -> emp.getJobHistory().get(0).getEmployer(), mapping(Employee::getPerson, toSet())));
    ...
    void indexByFirstEmployer() {
        ...
        Map<String, Set<Person>> result = employees.stream()
                .collect(groupingBy(employee -> employee.getJobHistory().get(0).getEmployer(), mapping(Employee::getPerson, toSet())));
        ...
        Map<String, Person> collect = employees.stream()
                .flatMap(employee ->
                        employee.getJobHistory().stream()
                                ...
                                .entrySet()
                                .stream()
                                .map(entry -> new Pair<Person, JobHistoryEntry>(employee.getPerson(), new JobHistoryEntry(entry.getValue(), "", entry.getKey())))
                )
                .collect(groupingBy(pair -> pair.getSecond().getEmployer(), collectingAndThen(maxBy(comparingInt(pair -> pair.getSecond().getDuration())),
                        pair -> pair.orElseThrow(NoSuchElementException::new).getFirst())));
